...
//#define RGB_MATRIX_KEYRELEASES // reacts to keyreleases (instead of keypresses) 对按键释放(而不是按键)作出反应
#define RGB_MATRIX_FRAMEBUFFER_EFFECTS // enable framebuffer effects 使framebuffer效果
#define RGB_DISABLE_TIMEOUT 0 // number of milliseconds to wait until rgb automatically turns off 等待RGB自动关闭的毫秒数  
#define RGB_DISABLE_AFTER_TIMEOUT 2 // OBSOLETE: number of ticks to wait until disabling effects 过时:在禁用效果之前等待的节拍数  
//#define RGB_MATRIX_LED_PROCESS_LIMIT (DRIVER_LED_TOTAL + 4) / 5 // limits the number of LEDs to process in an animation per task run (increases keyboard responsiveness)限制每次任务运行时动画中处理的led数量(增加键盘响应)  
#define RGB_MATRIX_LED_FLUSH_LIMIT 16 // limits in milliseconds how frequently an animation will update the LEDs. 16 (16ms) is equivalent to limiting to 60fps (increases keyboard responsiveness)以毫秒为单位限制动画更新led的频率。 16 (16ms)相当于限制到60fps(增加键盘响应)  
#define RGB_MATRIX_MAXIMUM_BRIGHTNESS 200 // limits maximum brightness of LEDs to 200 out of 255. If not defined maximum brightness is set to 255 将led的最大亮度限制为200 / 255。 如果没有定义，最大亮度设置为255  
//#define RGB_MATRIX_STARTUP_MODE RGB_MATRIX_CYCLE_LEFT_RIGHT // Sets the default mode, if none has been set 如果没有设置默认模式，则设置默认模式
#define RGB_MATRIX_STARTUP_HUE 0 // Sets the default hue value, if none has been set 如果没有设置，则设置默认的色调值  
#define RGB_MATRIX_STARTUP_SAT 255 // Sets the default saturation value, if none has been set 如果未设置饱和值，则设置默认饱和值  
#define RGB_MATRIX_STARTUP_VAL RGB_MATRIX_MAXIMUM_BRIGHTNESS // Sets the default brightness value, if none has been set 如果没有设置，则设置默认亮度值  
